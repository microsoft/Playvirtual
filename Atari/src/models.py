...
class PVCatDqnModel(torch.nn.Module):
    ...
    def forward(self, observation,
        ...
        if train:
            ...
            target_images = observation[self.time_offset:max_jumps + self.time_offset+1]\
            ...
            if self.forward_predict:
                ...
                if self.real_cycle:
                    ...
                    backward_latents = [cur_latents]
                    for i in reversed(range(1, 1 + self.cycle_step)):
                        actions = forward_actions.pop()
                        recon_latents = self.backward_dynamics_model(recon_latents, actions)
                        backward_latents.append(recon_latents)
                    # sample target
                    cycle_targets = observation[0].flatten(1, 2)   # [bs, 4, 84, 84]
                    cycle_targets = self.transform(cycle_targets, True)
                    if not self.momentum_encoder and not self.shared_encoder:
                        cycle_targets = cycle_targets[..., -1:, :, :]
                    with torch.no_grad() if self.momentum_encoder else dummy_context_mgr():
                        cycle_targets = self.target_encoder(cycle_targets)
                        if self.renormalize:
                            cycle_targets = renormalize(cycle_targets, -3)
                    cycle_targets = cycle_targets.clone().detach()
                    if self.space == 'z':
                        real_cycle_loss = torch.mean(F.mse_loss(recon_latents, 
                        cycle_targets, reduction='none'), dim=[1,2,3])
                    elif self.space == 'y':
                        real_cycle_loss = self.yspace_loss(recon_latents, 
                        cycle_targets, no_grad=False)
                else:
                    real_cycle_loss = 0
            else:
                for_pred_loss, real_cycle_loss = 0, 0 
            
            # backward prediction
            if self.backward_predict:
                if self.bp_mode == 'gt':
                    back_obs = observation[self.jumps].flatten(1, 2)
                    back_obs = self.transform(back_obs, augment=True)
                    back_latent = self.stem_forward(back_obs,
                                            prev_action[self.jumps],
                                            prev_reward[self.jumps])
                    ...
                    if torch.rand(1) > 0.5:
                        ...
                        back_latent = self.stem_forward(back_obs,
                                                prev_action[self.jumps],
                                                prev_reward[self.jumps])
                elif self.bp_mode == 'hybriddetach':
                    with torch.no_grad():
                        if torch.rand(1) > 0.5:
                            back_latent = cur_latents
                        else:
                    ...
                    raise NameError
                backward_latents = [back_latent]
                for i in reversed(range(1, 1 + self.jumps)):
                    cur_latents = backward_latents[-1]
                    cur_actions = prev_action[i]
                    cur_latents = self.backward_dynamics_model(cur_latents, cur_actions)
                    backward_latents.append(cur_latents)
                backward_latents = backward_latents[::-1]
                pred_latents = torch.stack(backward_latents, 1).flatten(0, 1)
                if self.bp_mode == 'detach':
                    back_pred_loss = self.byol_loss(pred_latents, target_latents, 
                    'spr', observation.size(1), 1+self.jumps, no_grad=True)
                else:
                    back_pred_loss = self.byol_loss(pred_latents, target_latents, 
                    'spr', observation.size(1), 1+self.jumps)
            else:
                back_pred_loss = 0

            # cycle loss
            if self.virtual_cycle:
                latents_t0 = latent[None].expand(self.aug_num, batch_size, 
                    latent.size(1), latent.size(2), latent.size(3)).flatten(0, 1) # (A,bs,..) -> (A*bs,..)
                actions_t0 = self.sample_cycle_actions(self.aug_type, 0, 
                    self.aug_num, batch_size, self.num_actions, 
                    prev_action, latent.device).flatten(0, 1)
                # sample cycle targets
                if self.space == 'z':
                    cycle_targets = latents_t0.clone().detach()
                elif self.space == 'y':
                    cycle_targets = observation[0].flatten(1, 2)   # [bs, 4, 84, 84]
                    cycle_targets = cycle_targets[None].expand(self.aug_num, 
                        cycle_targets.size(0), cycle_targets.size(1), 
                        cycle_targets.size(2), cycle_targets.size(3)).flatten(0, 1)   # (A*bs, 1, 4, 84, 84)
                    cycle_targets = self.transform(cycle_targets, True)
                    if not self.momentum_encoder and not self.shared_encoder:
                        cycle_targets = cycle_targets[..., -1:, :, :]
                    with torch.no_grad() if self.momentum_encoder else dummy_context_mgr():
                    ...
                    cycle_targets = cycle_targets.clone().detach()
                # forward
                forward_latents, forward_actions = [latents_t0], [actions_t0]
                for i in range(1, 1 + self.cycle_step):
                    ...
                    if i < self.cycle_step:
                        ...
                        i, self.aug_num, batch_size, self.num_actions, 
                        prev_action, latent.device).flatten(0, 1)
                        forward_actions.append(next_actions)
                # backward
                recon_latents = cur_latents
                backward_latents = [cur_latents]
                for i in reversed(range(1, 1 + self.cycle_step)):
                    actions = forward_actions.pop()
                    recon_latents = self.backward_dynamics_model(recon_latents, 
                                        actions)
                    backward_latents.append(recon_latents)
                if self.space == 'z':
                    virtual_cycle_loss = torch.mean(F.mse_loss(recon_latents, 
                    cycle_targets, reduction='none'), dim=[1,2,3])
                elif self.space == 'y':
                    virtual_cycle_loss = self.yspace_loss(recon_latents, 
                                            cycle_targets, no_grad=False)
                virtual_cycle_loss = virtual_cycle_loss.view(self.aug_num, 
                                            batch_size).mean(0)
            else:
            ...
            prediction_loss = [for_pred_loss, back_pred_loss]
            self.momentum_update()
            return log_pred_ps, pred_reward, real_cycle_loss, prediction_loss, virtual_cycle_loss
